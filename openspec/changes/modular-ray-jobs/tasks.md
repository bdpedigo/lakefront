...
- [ ] 0.1 Create `justfile` with initial commands: `just run <config>`, `just test-simple`, `just test-setup`, `just test-failure`
- [ ] 0.2 Add cluster commands: `just cluster-up-local`, `just cluster-down-local`, `just submit <config>`
- [ ] 0.3 Add utility commands: `just build-image`, `just dashboard`
...
- [ ] 1.4 Add Ray environment detection (local auto-init vs cluster connect)
- [ ] 1.5 **Test: `just run configs/simple.yaml`** — verify fan-out, results printed, exit 0
  - _Expect_: "Submitted N tasks… Completed N/N… 0 failures"

- [ ] 1.6 Implement setup phase: call setup(), pass ObjectRefs to tasks as kwargs
- [ ] 1.7 **Test: `just run configs/setup_example.yaml`** — verify shared state is passed
  - _Expect_: Each task prints that it received the shared ref; results collected

- [ ] 1.8 Implement error handling: continue on task failure, collect and report failures at end
- [ ] 1.9 **Test: `just run configs/failure_test.yaml`** — verify continue-on-failure
  - _Expect_: "Completed 7/10… 3 failures:" followed by item IDs and error messages, exit 1

## 2. Job Authoring

- [ ] 2.1 Create `jobs/square.py` — simple @ray.remote job: takes an int, returns int**2
- [ ] 2.2 Create `jobs/with_setup.py` — job with setup phase (shared lookup table via ray.put)
- [ ] 2.3 Create `jobs/flaky.py` — job that fails on certain items (for failure testing)

## 3. Insertion Authoring

- [ ] 3.1 Create `insertions/range_items.py` — returns `list(range(N))` for testing
- [ ] 3.2 Create `insertions/flaky_items.py` — returns items where some are designed to trigger failures

## 4. YAML Configs

- [ ] 4.1 `configs/simple.yaml` — wires `jobs.square.process` + `insertions.range_items.get_items`
- [ ] 4.2 `configs/setup_example.yaml` — wires `jobs.with_setup.process` + setup + insertion
- [ ] 4.3 `configs/failure_test.yaml` — wires `jobs.flaky.process` + `insertions.flaky_items.get_items`

## 5. Local Kind Cluster Testing

- [ ] 5.1 `just cluster-up-local` — start kind cluster, load Docker image, deploy RayCluster
- [ ] 5.2 `just submit configs/simple.yaml` — submit job via `ray job submit` to local cluster
  - _Expect_: Same output as local run, but executed on kind cluster
- [ ] 5.3 `just dashboard` — port-forward Ray dashboard, open browser
- [ ] 5.4 `just cluster-down-local` — clean teardown
...
- [ ] 6.4 `just cluster-up` / `just cluster-down` for remote cluster
- [ ] 6.5 **Test: `just submit configs/simple.yaml`** on remote cluster — verify same behavior as local
